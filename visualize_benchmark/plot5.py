...
for tuples in ["10", "100", "1000", "10000", "100000", "1000000"]:
    filesArray.append([])
    for i in [1,2,3,4,5]:
        filesArray[-1].append(tuples + "_ad_ids_" + str(i) + ".csv")
...
df["y_values"].plot(kind='bar', yerr=errors, align='center', capsize=10, zorder=0, color=["#3193C6", "#05AD97", "#AAC56C", "#F7AB13", "#CD4E38", "#A553EC"], error_kw=dict(ecolor='#303030'))

for i, rows in df.iterrows():
    plt.annotate(round(rows["y_values"]), xy=(i - 0.15, rows["y_values"] + 0.5), rotation=0, zorder=5, fontsize=small_font_size, ha="center")

# General stuff for latency diagramm
plt.xticks([0,1,2,3,4,5], x_labels)
plt.xticks(rotation=0, ha='center')

plt.xlabel('Number of unique keys', fontsize=big_font_size)
...
plt.tight_layout()
plt.savefig(pathlib.Path("plot5.svg"), format="svg")
plt.close()
