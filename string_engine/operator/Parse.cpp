...
std::string _Parse::getCode()
{
    ...
    for (int i = 0; i < _schema->getVariables().size() - 1; i++) {
        ...
        stream << parse("*pos != ','", "pos", datatype->getDataType(), datatype->getVariableIdentifier());
    }
    auto lastDatatype = _schema->getVariables().back();
    stream << parse("pos != end", "pos", lastDatatype->getDataType(), lastDatatype->getVariableIdentifier()) << std::endl;
    stream << "{yield}";
    return stream.str();
};
...
std::string _Parse::parse(std::string whileCondition, std::string posName, std::string variableType, std::string variableName)
{
    return fmt::format(
